...
// write discriminator enable
void TCBwrite_DISC_ENABLE(int sid, int mid, int *data)
{
  int value;
  int ch;
  ...
  TCBWrite(sid, mid, 0x20000002, value);
}

// read discriminator enable
void TCBread_DISC_ENABLE(int sid, int mid, int *data)
{
  int value;
  int ch;

  value = TCBRead(sid, mid, 0x20000002);

  for (ch = 0; ch < 32; ch++) 
    data[ch] = (value >> ch) & 0x1;
}
...
// write TDC calibration channel
void TCBwrite_TDC_CAL_CH(int sid, int mid, int data)
{
  TCBWrite(sid, mid, 0x2000000C, data - 1);
}

// read TDC calibration channel
int TCBread_TDC_CAL_CH(int sid, int mid)
{
  return (TCBRead(sid, mid, 0x2000000C) - 1);
}

// start TDC calibration
void TCBstart_TDC_CAL(int sid, int mid)
{
  TCBWrite(sid, mid, 0x2000000D, 0x01);
}

// stop TDC calibration
void TCBstop_TDC_CAL(int sid, int mid)
{
  TCBWrite(sid, mid, 0x2000000D, 0x00);
}

// read TDC calibration status
int TCBread_TDC_CAL_STATUS(int sid, int mid)
{
...
void TCBinit_BIC_ADC(int sid, int mid)
{
  int retry;
  int all_aligned;
  int ch;
...
void TCBinit_JBNU_ADC(int sid, int mid)
{
  int retry;
  int all_aligned;
  int ch;
...
void TCBwrite_HV(int sid, int mid, int ch, float data)
{
	...
	present_val = TCBRead(sid, mid, addr);
	diff_val = target_val - present_val;
	value = present_val;
...
void MINITCB_V2write_RL(int sid, int mid, int data)
{
	TCBWrite(sid, mid, 0x20000002, data);
	return;
}

int MINITCB_V2read_RL(int sid, int mid)
{
	return TCBRead(sid, mid, 0x20000002);
}

void MINITCB_V2write_DRAMON(int sid, int mid, int data)
{
	int status;
	...
	{
		// check DRAM is on
		status = TCBRead(sid, mid, 0x20000003);

		// when DRAM is on now, turn it off
		if (status) TCBWrite(sid, mid, 0x20000003, 0);

		// turn on DRAM
		TCBWrite(sid, mid, 0x20000003, 1);

		// wait for DRAM ready
		status = 0;
		while (!status) status = TCBRead(sid, mid, 0x20000003);
	}
	// trun off DRAM
	else TCBWrite(sid, mid, 0x20000003, 0);

	return;
}

int MINITCB_V2read_DRAMON(int sid, int mid)
{
	return TCBRead(sid, mid, 0x20000003);
}

void MINITCB_V2write_DACOFF(int sid, int mid, int ch, int data)
{
	int addr = 0x20000004 + (((ch - 1) & 0xFF) << 16);
...
int MINITCB_V2read_DACOFF(int sid, int mid, int ch)
{
	int addr = 0x20000004 + (((ch - 1) & 0xFF) << 16);
	return TCBRead(sid, mid, addr);
}

void MINITCB_V2measure_PED(int sid, int mid, int ch)
{
	int addr = 0x20000005 + (((ch - 1) & 0xFF) << 16);
	TCBWrite(sid, mid, addr, 0);

	return;
}

int MINITCB_V2read_PED(int sid, int mid, int ch)
{
	int addr = 0x20000006 + (((ch - 1) & 0xFF) << 16);
	return TCBRead(sid, mid, addr);
}

void MINITCB_V2write_DLY(int sid, int mid, int ch, int data)
{
	int addr;
	int value;
...
int MINITCB_V2read_DLY(int sid, int mid, int ch)
{
	int data;
	int addr;
	int value;
	...
	addr = 0x20000007 + (((ch - 1) & 0xFF) << 16);
	value = TCBRead(sid, mid, addr);
	data = (value >> 10)*1000 + (value&0X3FF);

	return data;
}

void MINITCB_V2write_THR(int sid, int mid, int ch, int data)
{
	int addr;
...
int MINITCB_V2read_THR(int sid, int mid, int ch)
{
	int addr = 0x20000008 + (((ch - 1) & 0xFF) << 16);
	return TCBRead(sid, mid, addr);
}

void MINITCB_V2write_POL(int sid, int mid, int ch, int data)
{
	int addr = 0x20000009 + (((ch - 1) & 0xFF) << 16);
	TCBWrite(sid, mid, addr, data);
}

int MINITCB_V2read_POL(int sid, int mid, int ch)
{
	int addr = 0x20000009 + (((ch - 1) & 0xFF) << 16);
	return TCBRead(sid, mid, addr);
}

void MINITCB_V2write_AMODE(int sid, int mid, int ch, int data)
{
	int addr = 0x2000000B + (((ch - 1) & 0xFF) << 16);
	TCBWrite(sid, mid, addr, data);
	return;
}

int MINITCB_V2read_AMODE(int sid, int mid, int ch)
{
	int addr = 0x2000000B + (((ch - 1) & 0xFF) << 16);
	return TCBRead(sid, mid, addr);
}

void MINITCB_V2send_ADCRST(int sid, int mid)
{
	TCBWrite(sid, mid, 0x20000017, 0);
	return;
}

void MINITCB_V2send_ADCCAL(int sid, int mid)
{
	TCBWrite(sid, mid, 0x20000018, 0);
	return;
}

void MINITCB_V2write_ADCDLY(int sid, int mid, int ch, int data)
{
	int addr = 0x20000019 + (((ch - 1) & 0xFF) << 16);
	TCBWrite(sid, mid, addr, data);
	return;
}

void MINITCB_V2write_ADCALIGN(int sid, int mid, int data)
{
	int addr = 0x2000001A; 
	TCBWrite(sid, mid, addr, data);
	return;
...
int MINITCB_V2read_ADCSTAT(int sid, int mid)
{
	return TCBRead(sid, mid, 0x2000001A);
}

void MINITCB_V2_ADCALIGN(int sid, int mid)
{
	int ch, dly, value;
	...
	MINITCB_V2send_ADCRST(sid, mid); 
	usleep(500000);
	MINITCB_V2send_ADCCAL(sid, mid); 
	MINITCB_V2write_ADCALIGN(sid, mid, 1);
	...
	for (ch = 1; ch <= 4; ch++) {
		...
		for (dly = 0; dly < 32; dly++) {
			MINITCB_V2write_ADCDLY(sid, mid, ch, dly); 
			value = (MINITCB_V2read_ADCSTAT(sid, mid) >> (ch -1)) & 0x1; 
		...
		// set delay
		MINITCB_V2write_ADCDLY(sid, mid, ch, gdly); 
		printf("ch%d calibration delay = %d\n", ch, gdly);
	}

	MINITCB_V2write_ADCALIGN(sid, mid, 0); 
	MINITCB_V2send_ADCCAL(sid, mid);

	return;
}

void MINITCB_V2write_DRAMDLY(int sid, int mid, int ch, int data)
{
	int addr = 0x20000022 + ((ch & 0xFF) << 16);
	TCBWrite(sid, mid, addr, data);
	return;
}

void MINITCB_V2write_DRAMBITSLIP(int sid, int mid, int ch)
{
	int addr = 0x20000023 + ((ch & 0xFF) << 16);
	TCBWrite(sid, mid, addr, 0);
	return;
}

void MINITCB_V2write_DRAMTEST(int sid, int mid, int data)
{
	TCBWrite(sid, mid, 0x20000024, data);
	return;
}

int MINITCB_V2read_DRAMTEST(int sid, int mid, int ch)
{
	int addr = 0x20000024 + ((ch & 0xFF) << 16);
	return TCBRead(sid, mid, addr);
}

void MINITCB_V2_ADCALIGN_DRAM(int sid, int mid)
{
	int ch;
	int dly;
	int value;
	...
	// turn on DRAM    
	MINITCB_V2write_DRAMON(sid, mid, 1);

	// enter DRAM test mode
	MINITCB_V2write_DRAMTEST(sid, mid, 1);

	// send reset to iodelay  
	MINITCB_V2send_ADCCAL(sid, mid);

	// fill DRAM test pattern
	MINITCB_V2write_DRAMTEST(sid, mid, 2);
	...
	for (ch = 0; ch < 8; ch++)
	{
		...
		for (dly = 0; dly < 32; dly++) {
			// set delay
			MINITCB_V2write_DRAMDLY(sid, mid, ch, dly);

			// read DRAM test pattern
			MINITCB_V2write_DRAMTEST(sid, mid, 3);
			value = MINITCB_V2read_DRAMTEST(sid, mid, ch) & 0xFFFFFFFF;
		...
		// set delay
		MINITCB_V2write_DRAMDLY(sid, mid, ch, gdly); 
		...
		for (bitslip = 0; bitslip < 4; bitslip++) {
			...
			if (value == 0xFFAA5500) {
				...
				aflag = 0;
				MINITCB_V2write_DRAMBITSLIP(sid, mid, ch);
			}
		}
	...
	// exit DRAM test mode
	MINITCB_V2write_DRAMTEST(sid, mid, 0);
	return;
}
